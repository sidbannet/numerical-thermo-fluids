...
class CoolPropEOS(AbstractEOS):
    ...
    def __init__(self, fluid: str = 'Water', backend: str = 'HEOS'):
        ...
        try:
            ...
            raise ImportError("CoolProp is not installed. Please install it to use CoolPropEOS.")
    ...
    def _hem_speed_of_sound(self, P_array: np.ndarray, s_array: np.ndarray) -> np.ndarray:
        """Calculate the Homogeneous Equilibrium Speed of Sound using an isentropic compressibility approach."""
        # This will operate on arrays of properties that are confirmed to be in the two-phase region.
        a = np.zeros_like(P_array)
    ...
    def speed_of_sound(self, rho: np.ndarray, e: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if len(idx_1phase) > 0:
            a[idx_1phase] = self.CP.PropsSI('A', 'D', rho_flat[idx_1phase], 'U', e_flat[idx_1phase], self.fluid)
            
        # Two Phase calculation (HEM fallback)
        if len(idx_2phase) > 0:
            P_2ph = self.CP.PropsSI('P', 'D', rho_flat[idx_2phase], 'U', e_flat[idx_2phase], self.fluid)
            S_2ph = self.CP.PropsSI('S', 'D', rho_flat[idx_2phase], 'U', e_flat[idx_2phase], self.fluid)
